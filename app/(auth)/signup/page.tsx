...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // signup() will redirect to /dashboard or /verify-email — only returns on error
    ...
    <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden">
      ...
      >
        ...
        <div className="rounded-2xl border border-border bg-card p-8 shadow-xl shadow-black/5">
          <Button variant="outline" className="w-full mb-6 gap-2" onClick={handleGoogle} loading={googleLoading}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Input name="full_name" type="text" label="Full Name" placeholder="John Doe" icon={<User className="h-4 w-4" />} required />
            <Input name="email" type="email" label="Email" placeholder="you@example.com" icon={<Mail className="h-4 w-4" />} required />
            <Input name="password" type="password" label="Password" placeholder="Min. 8 characters" icon={<Lock className="h-4 w-4" />} required minLength={8} />
            <Input name="confirm_password" type="password" label="Confirm Password" placeholder="Repeat your password" icon={<Lock className="h-4 w-4" />} required />
