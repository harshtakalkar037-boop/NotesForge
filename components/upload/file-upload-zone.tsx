...
import {
  Upload, X, FileText, Image as ImageIcon, Link as LinkIcon,
  CheckCircle2, AlertCircle, File, Presentation, Sparkles,
...
const FILE_TYPE_ICONS: Record<string, React.ElementType> = {
  ...
  txt: FileText, image: ImageIcon, link: LinkIcon,
};
...
interface QueuedFile {
  ...
  result?: {
    ...
    summary: string;
    tags: string[];
    fileUrl?: string;   // signed Supabase URL
    fileType?: string;
  };
}
...
export function FileUploadZone() {
  ...
  const uploadFile = useCallback(async (queued: QueuedFile) => {
    updateFile(queued.id, { status: "uploading", progress: 25 });
      ...
      setFiles((prev) =>
        prev.map((f) =>
          f.id === queued.id && f.status === "uploading"
            ? { ...f, progress: Math.min(f.progress + 8, 70) }
            : f
        )
      );
    }, 400);
    ...
    clearInterval(timer);
    updateFile(queued.id, { status: "processing", progress: 80 });
    ...
    updateFile(queued.id, {
      status: "complete",
      progress: 100,
      result: result.note ? { ...result.note, fileUrl: result.note.fileUrl ?? undefined } : undefined,
    });
  }, [updateFile]);
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-sm text-muted-foreground mb-4">or click to browse — uploads & AI processes instantly</p>
        ...
        {files.length > 0 && (
          ...
          >
            ...
            {files.map((file) => {
              ...
              const isImage = IMAGE_EXTS.includes(getFileExtension(file.name));
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 p-3">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {file.status === "processing" && (
                          <span className="text-xs text-violet-400 flex items-center gap-1">
                            <Sparkles className="h-3 w-3 animate-pulse" />AI processing...
                          </span>
                        )}
                    ...
                    {/* Open file button (when complete and URL exists) */}
                    {file.status === "complete" && file.result?.fileUrl && (
                      <a
                  ...
                  {/* Image preview (large) after upload */}
                  {file.status === "complete" && isImage && (file.previewUrl || file.result?.fileUrl) && (
                    <div className="px-3 pb-3 pt-0">
                      <a
                        href={file.result?.fileUrl ?? file.previewUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="block rounded-lg overflow-hidden border border-border hover:border-indigo-500/40 transition-colors"
                      >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src={file.previewUrl ?? file.result?.fileUrl}
                          alt={file.name}
                  ...
                  {file.status === "complete" && file.result?.summary && (
                    <div className="px-3 pb-3 border-t border-border/50 pt-2">
                      <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-lg p-3">
                        ...
                        <p className="text-xs text-muted-foreground leading-relaxed">{file.result.summary}</p>
